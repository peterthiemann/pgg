  ...
  (lambda (size f)
    ...
    (if (zero? size)
        ...
        (let ((x (make-vector size (f 0))))
          (let loop ((i 1))		;pjt: recur --> let
            (if (< i size)		;pjt: when --> if
		(begin
		  (vector-set! x i (f i))
		  (loop (+ i 1)))))
          x))))
	...
	(let loop-*-
		...
		(let loop-**-
			...
			(b-folder first
				...
				(loop-*- (fold out (lambda (x y) (cons x y)) rest) ;pjt: eta expanded
				    new-b-state
				    new-t-state
				    accross))
			    accross)))))))
	...
	(let ((res
		...
		(lambda (m)
			...
			(gnatural-for-each size
				...
				(let ((from-f
					    (vector-ref res f)))
				    (if (vector-ref from-f m) ;pjt: when --> if
					(gnatural-for-each size
					    (lambda (t)
					      (if (vector-ref from-m t)	;pjt: when --> if
